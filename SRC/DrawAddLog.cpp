    ...
    if (m.x >= 300 && m.x <= 380 && m.y >= 50 && m.y <= 70) {  // 挂号号码
      ...
      if (m.uMsg == WM_LBUTTONDOWN) {
        ...
        if (LEGAL_DATA == IsDate(buf_CDate) &&
          ...
          temp->costByNow = constructSingleCost(buf_Cost);
          now = temp->costByNow;
  ...
  while (1) {
    ...
    if (m.x >= 90 && m.x <= 410 && m.y >= 88 && m.y <= 118) {  // 科室
      ...
      if (m.uMsg == WM_LBUTTONDOWN) {
        ...
        SetDate(buf_stTime, &stTime.month, &stTime.day,
          &stTime.hour, &stTime.minute);
        SetDate(buf_edTime, &edTime.month, &edTime.day,
          &edTime.hour, &edTime.minute);
...
void printData(char* buf, DiagnosisRecord* res) {
  ...
  sprintf(cache, "%d", res->doctorInfo.level);
  ...
  if (res->diagnosisSituation.setFlag == 2) {
    ...
    fillrectangle(242, 297, 253, 308);
    setfillcolor(RGB(210, 210, 210));
    PrescribeRecord * temp = 
      &res->diagnosisSituation.diagnosisSituationInfo.prescribeRecord;
      ...
      if (m.uMsg == WM_LBUTTONDOWN) {
        ...
        if (LEGAL_DATA == IsRegisterId(buf)) {
          ...
          if (res) {
            ...
            switch (res->diagnosisSituation.setFlag) {
            case 0:
              past = res->diagnosisSituation.diagnosisSituationInfo.
                checkRecord.totalCost;
              flag = 0;
              break;
            case 1:
              past = res->diagnosisSituation.diagnosisSituationInfo.
                prescribeRecord.totalCost;
              flag = 1;
              break;
            case 2:
              past = res->diagnosisSituation.diagnosisSituationInfo.
                inHospitalRecord.costByNow;
              flag = 2;
              break;
            default:
              break;
